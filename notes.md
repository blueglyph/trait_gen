...
## 2. Generics and cross-product generators

Allows the substitution of generic parameters, which allows cross-product generators
for trait implementations, and which will allow the use of the attribute on
generic type implementations later.

But this removes the check on generics, so internal generics will be misinterpreted
if their parameters have the same name as the `trait_gen` attribute parameters.
...
Quick tour of the different generic forms in Rust.

* **Generic functions**

   ```rust
   pub fn add_mod<T>(a: T, b: T, m: T) -> T
       where T: Add<Output = T> + Rem<Output = T>
   {
       (a + b) % m
   }
   ```
  
  Quite simple to use, but may require a series of trait bounds. Not possible when there
  are methods that don't belong to a trait.
...
* **Generic `struct` / `enum`**
  ...
  // Cannot implement the same method for f32, even when adding 'Integer' as trait bound in
  // the generic implementation:
  
  impl Vector<f32> {
      pub fn length_f32(&self) -> f64 {
          (self.0 * self.0 + self.1 * self.1).sqrt() as f64
      }
  }
  ```
  
  If there are many types, it's possible to use a generic implementation, like below. However,
  it is not allowed to add an individual implementation for a type that is included in the
  generic implementation: here, any type which has `Copy` and `Into<f64>`. So for instance,
  it is not allowed to implement the `length()` method for `Vector<f32>`, the generic one will
  have to be used. Here, we chose another name.
...
* **Trait with associated type**
  ...
  impl<T> Sqr for T where T: Copy + Mul<Self, Output = T> {
      type Output = T;
  ...
  ```
  This is a blanket implementation of the `Sqr` trait for all types that satisfy the
  bounds. The trait method doesn't have a default implementation because it would be
  too constraining and would forbid its implementation for the `Vector` type below:

  ```rust
  fn sqr(self) -> <Self as Sqr>::Output
      where Self: Copy + Mul<Self, Output = <Self as Sqr>::Output>
  {
      self * self
  }
  ```

  It is still possible to implement types that are not in conflict with the blanket
  implementation. For instance, the type below doesn't have `Copy`, nor `Mul<Output = f64>`.

  ```rust
  pub struct Vector<T>(T, T);
  
  impl Sqr for Vector<f64> {
      type Output = f64;
  
      fn sqr(self) -> f64 {
          self.0.sqr() + self.1.sqr()
      }
  }
  ```
...
* **Generic trait**
  ...
  This is quite similar to the trait with associated type, except it is now expressed
  as a generic `<U>`, and it cannot be referenced by bounds:
  ...
  impl<T, U> Sqr<U> for T where T: Copy + Mul<Self, Output = U> {
      fn sqr(self) -> U {
          self * self
      }
  }
  
  pub struct Vector<U>(U, U);
  
  impl Sqr<f64> for Vector<f64> {
      fn sqr(self) -> f64 {
          self.0.sqr() + self.1.sqr()
      }
  }
