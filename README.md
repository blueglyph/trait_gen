...
_Note: using the letter "`T`" is not mandatory, any type path will do. For example, `gen::Type` is fine too. But to make it easy to read and similar to a generic implementation, using short identifiers is preferred._ 
...
But the result is harder to read than native code, and the developer must write the macro declaration each time, along with its pattern, and translate a few elements like the parameters (`$t`). Moreover, IDEs can't often provide contextual help or apply refactoring in the macro code.

It's also quite annoying and unhelpful to get this result when looking for the definition of a method when it has been generated by a declarative macro:
